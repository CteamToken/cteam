...
// Copyright (c) 2017 The Christian Team developers
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CTEAMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CTEAMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CTEAMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cteam-miner");
...
//            }
        ...
        LogPrintf("Running CTEAMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
