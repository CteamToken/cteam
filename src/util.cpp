...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cteam" is a composite category enabling all cteam-related debug output
            if (ptrCategory->count(string("cteam"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cteam.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cteam.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cteam.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cteamd.pid"));
